...
class Common_util_pub
{
	...
	{		
        ...
        $data = curl_exec($ch);
 		curl_close($ch);
		//file_put_contents(APP_ROOT_PATH."/sjmapi/log/log_xml.txt",print_r($data,1));
  		//返回结果
		if($data)
		{
			curl_close($ch);
...
// 		$jsApiObj["appId"] = "wxbe5ad809db64f3d6";
		$timeStamp = time();
	  ...
	  //$this->parameters ='{timestamp:'. $jsApiObj["timeStamp"].',nonceStr:"'. $jsApiObj["nonceStr"].'",package:"'.$jsApiObj["package"].'",signType:"'.$jsApiObj["signType"].'",paySign:"'.$jsApiObj["paySign"].'",  success: function (res) { if(res.errMsg=="chooseWXPay:ok"){alert("恭喜您支付成功");}}}';
