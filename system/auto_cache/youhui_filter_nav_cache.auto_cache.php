<?php
//优惠券模块的筛选切换单菜
class youhui_filter_nav_cache_auto_cache extends auto_cache{
	public function load($param)
	{
		...
		if($result===false||IS_DEBUG)
		{
			...
			$bquan_list[] = array("url"=>url("index","youhuis",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
			...
			foreach($bquan_list_res as $k=>$v)
			{
					...
					unset($tmp_url_param['qid']);
					$durl = url("index","youhuis",$tmp_url_param);
					$v['url']=$durl;
			...
			if($deal_area_id>0)
			{
					...
					if($squan_list_res)
					{
						...
						$tmp_url_param = $url_param;
						unset($tmp_url_param['qid']);
						$squan_list[] = array("url"=>url("index","youhuis",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
					}
...
// 			$bcate_list[]= array("name"=>$GLOBALS['lang']['ALL'],"qid"=>$deal_quan_id);
			...
			$bcate_list[] = array("url"=>url("index","youhuis",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
			...
			{		
						...
						unset($tmp_url_param['tid']);
						$v['url'] = url("index","youhuis",$tmp_url_param);
...
// 						$bcate_list[$k+1]['count'] = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal where is_effect = 1 and is_delete = 0 and is_shop = 0 and (end_time = 0 or end_time > '".NOW_TIME."') $condition ");
			...
			if($deal_cate_id>0)
			{
					...
					if($scate_list_res)
					{
						...
						$tmp_url_param = $url_param;
						unset($tmp_url_param['tid']);
						$scate_list[] = array("url"=>url("index","youhuis",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
					}
