...
class store_filter_nav_cache_auto_cache extends auto_cache{
	...
	{
		$param = array("city_id"=>$param['city_id'],"cid"=>$param['cid'],"tid"=>$param['tid'],"aid"=>$param['aid'],"qid"=>$param['qid'],"supplier_id"=>$param['supplier_id']); //重新定义缓存的有效参数，过滤非法参数
		
		$key = $this->build_key(__CLASS__,$param);
		//传入参数 city_id(城市)  cid(分类ID) tid(子分类ID) aid(区域ID) qid(商圈ID) supplier_id(商户ID)
		...
		if($result===false||IS_DEBUG)
		{
			...
			$param['supplier_id'] = intval($GLOBALS['db']->getOne("select id from ".DB_PREFIX."supplier where id = ".intval($param['supplier_id'])));
			...
			$deal_cate_id = $param['cid'];
			$deal_type_id = $param['tid'];
			$deal_area_id = $param['aid'];
			$deal_quan_id = $param['qid'];	
			$supplier_id = $param['supplier_id'];		

			$url_param = array("cid"=>$param['cid'],"tid"=>$param['tid'],"aid"=>$param['aid'],"qid"=>$param['qid'],"supplier_id"=>$param['supplier_id']);
			...
			$bquan_list[] = array("url"=>url("index","stores",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
			...
			foreach($bquan_list_res as $k=>$v)
			{
					...
					unset($tmp_url_param['qid']);
					$durl = url("index","stores",$tmp_url_param);
					$v['url']=$durl;
			...
			if($deal_area_id>0)
			{
					...
					if($squan_list_res)
					{
						...
						$tmp_url_param = $url_param;
						unset($tmp_url_param['qid']);
						$squan_list[] = array("url"=>url("index","stores",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
					}
...
// 			$bcate_list[]= array("name"=>$GLOBALS['lang']['ALL'],"qid"=>$deal_quan_id);
			...
			$bcate_list[] = array("url"=>url("index","stores",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
			...
			{		
						...
						unset($tmp_url_param['tid']);
						$v['url'] = url("index","stores",$tmp_url_param);
...
// 						$bcate_list[$k+1]['count'] = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal where is_effect = 1 and is_delete = 0 and is_shop = 0 and (end_time = 0 or end_time > '".NOW_TIME."') $condition ");
			...
			if($deal_cate_id>0)
			{
					...
					if($scate_list_res)
					{
						...
						$tmp_url_param = $url_param;
						unset($tmp_url_param['tid']);
						$scate_list[] = array("url"=>url("index","stores",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
					}
