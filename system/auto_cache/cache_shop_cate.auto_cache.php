...
class cache_shop_cate_auto_cache extends auto_cache{
	...
	{
		$param = array("pid"=>$param['pid'],"all"=>$param['all']);
		...
		if($cate_cache===false)
		{
			...
			elseif($param['pid']==0)
			{
				$cate_cache_res = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."shop_cate where is_effect = 1 and  pid =0 and is_delete = 0");
			}
			else
			{
				$pid = $param['pid'];
