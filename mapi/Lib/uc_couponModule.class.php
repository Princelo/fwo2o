...
class uc_couponModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		else
		{
			...
			if($tag==2)//未使用
			{
				$ext_condition = " and c.confirm_time = 0 and (c.end_time = 0 or (c.end_time>0 and c.end_time > $now))";
			}
			if($tag==3)//已失效
			{
				$ext_condition = " and (c.confirm_time <> 0 or (c.end_time < $now and c.end_time > 0))";
			}
