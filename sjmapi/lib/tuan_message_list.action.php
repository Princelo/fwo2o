...
// 		$message_re=m_get_message_list($limit," m.rel_table = 'deal' and m.rel_id=".$tuan_id." and m.is_buy = 1",0);/*购买评论*/
		...
		$message_re = get_dp_list($limit,$param=array("deal_id"=>$tuan_id,"youhui_id"=>0,"event_id"=>0,"location_id"=>0,"tag"=>""),"","");
		...
		$dp_info = load_dp_info(array("deal_id"=>$tuan_id));
		require_once APP_ROOT_PATH."system/model/deal.php";
		$deal_info = get_deal($tuan_id);
		$root['name'] = $deal_info['name'];
		//星级点评数
...
// 				$root['allow_dp'] = 1;
// 			}
			$dp_status = check_dp_status($user_id,array("deal_id"=>$tuan_id));
			if($dp_status['status'])
				$root['allow_dp'] = 1;
		}
