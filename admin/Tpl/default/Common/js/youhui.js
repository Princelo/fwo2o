function init_sub_cate()
{
	...
	{		
		$.ajax({ 
			url: ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"=load_sub_cate&cate_id="+cate_id+"&edit_type="+edit_type+"&id="+id, 
			data: "ajax=1",
			dataType: "json",
...
function init_supplier_location()
{
	...
	{		
		$.ajax({ 
			url: ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"=load_supplier_location&supplier_id="+supplier_id+"&edit_type="+edit_type+"&id="+id, 
			data: "ajax=1",
			dataType: "json",
			success: function(obj){
				if(obj.status)
